...
DO NOT USE USE PhD award v2 Now keeping this only for archival purposes
...
# Load the Excel file
file_path = '/Users/co25936/Desktop/PER/IPEDS/GSS_IPEDS_Combined_file_test.xlsx'  # Replace with your actual file path
...
for unitid in unitid_list:
    ...
    # Check if there is an AWLEVEL of 7 but not 17
    university_data = data[(data['UNITID'] == unitid)]
    has_awlevel_7 = university_data['AWLEVEL'].eq(7).any()
    has_awlevel_17 = university_data['AWLEVEL'].eq(17).any()


    # If there is an AWLEVEL of 7 but not 17, set pa_value to NaN
    if has_awlevel_7 and not has_awlevel_17:
        for year in range(2000, 2024):
            # Append the original row with NaN values for PA_Value and Retention_Value
            results.append({
                'UNITID': unitid,
                'Year': year,
                'PA_Value': float('nan'),  # Set PA_Value to NaN
                'Retention_Value': float('nan'),  # Retention value can also be set to NaN
                'Years_Used': None,  # You can set this to None or an empty list
                'AWLEVEL': 17  # Set AWLEVEL to 17 for this new row
            })
...
for year in range(2000, 2024):
    ...
    # Debugging: Print the relevant data
    print(f"Year: {year}")
    print("Relevant Years Data:")
    print(relevant_years)
    print("Relevant Years Plus 6 Data:")
    print(relevant_years_plus_6)

    # Check if relevant_years or relevant_years_plus_6 contains NaN or does not have AWLEVEL of 17
    has_nan_relevant_years = relevant_years.isna().any().any()
    has_nan_relevant_years_plus_6 = relevant_years_plus_6.isna().any().any()
    has_awlevel_17_relevant_years = 17 in relevant_years['AWLEVEL'].values
    has_awlevel_17_relevant_years_plus_6 = 17 in relevant_years_plus_6['AWLEVEL'].values

    print(f"Has NaN in Relevant Years: {has_nan_relevant_years}")
    print(f"Has NaN in Relevant Years Plus 6: {has_nan_relevant_years_plus_6}")
    print(f"HAS AWLEVEL 17 for the year: {has_awlevel_17}")
    print(f"Has AWLEVEL 17 in Relevant Years: {has_awlevel_17_relevant_years}")
    print(f"Has AWLEVEL 17 in Relevant Years Plus 6: {has_awlevel_17_relevant_years_plus_6}")

    if has_nan_relevant_years or has_nan_relevant_years_plus_6 or \
       not has_awlevel_17_relevant_years or not has_awlevel_17_relevant_years_plus_6:
        pa_value = float('nan')  # Set PA_Value to NaN
        numerator = float('nan')  # Set Numerator to NaN
        denominator = float('nan')  # Set Denominator to NaN
    else:
        ...
        if denominator != 0:
            ...
            pa_value = float('nan')  # Set PA_Value to NaN if denominator is zero
...
results_file_path = os.path.join(output_dir, 'pa_results_test.xlsx')
...
for unitid in unitid_list:
    ...
    university_data = merged_df[(merged_df['UNITID'] == unitid)]
# Iterate over each year from 2000 to 2023
    for year in range(2000, 2024):
        # Get the relevant rows for the current year and the range defined by x
        relevant_years = merged_df[(merged_df['Year'] >= year - x) & 
                                          (merged_df['Year'] <= year + x)]
        relevant_years_plus_6 = merged_df[(merged_df['Year'] >= year + 6 - x) & 
                                          (merged_df['Year'] <= year + 6 + x)]
        ...
        if denominator != 0:
            ...
            # Calculate T_n and T_{n+1}
            T_n = relevant_years['ft_tot_all_races_v'].sum()  # Replace with the actual column name for T_n
            T_n_plus_1 = merged_df[merged_df['Year'] == year + 1]['ft_tot_all_races_v'].sum()  # Replace with the actual column name for T_{n+1}
            F_n_plus_1 = merged_df[merged_df['Year'] == year + 1]['ft_frst_tot_all_races_v'].sum()  # F_{n+1}
            numerator_plus_1 = merged_df[merged_df['Year'] == year + 1]['CTOTALT'].sum() # CTOTAL FOR NEXT YEAR
...
# Add the legend to the plot
plt.legend(title='University UNITID', bbox_to_anchor=(0, 1.2), loc='lower left')  # Adjust the position as needed
...
# CURENTLY WORKING AND GIVING CORRECT OUTPUT TO DO
# INCLUDE NEW ROWS
# TEST MATH
